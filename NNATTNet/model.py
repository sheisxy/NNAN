...
def build(total_batch):
    ...
    with tf.variable_scope('attention_module') as scope:
        A_W = tf.Variable(
            tf.truncated_normal([FLAGS.input_size,FLAGS.input_size, FLAGS.A_node], stddev=0.1, seed=FLAGS.set_seed))
        A_b = tf.Variable(
        ...
        for i in range(FLAGS.input_size):
            attention_FC = tf.matmul(X, A_W_unstack[i]) + A_b_unstack[i]
            attention_out = tf.nn.sigmoid(attention_FC)
    ...
    with tf.variable_scope("learning_module") as scope:
        ...
        variable_averages = tf.train.ExponentialMovingAverage(
            FLAGS.moving_average_decay, global_step)    # 指数加权平均
        ...
        average_O = tf.sigmoid(average_O)
    with tf.name_scope("Loss") as scope:
        ...
        auc_val, auc_op = tf.metrics.auc(tf.argmax(Y, 1), tf.reduce_max(average_O, 1))
        aupr_val, aupr_op = tf.metrics.auc(Y[:, 0], average_O[:, 0], curve='PR')
    ...
    with tf.control_dependencies([optimizer, variable_averages_op]):
        train_op = tf.no_op(name='train') # after executing optimizer and variable_averages_op do nothing
    for op in [train_op, A]:
        tf.add_to_collection('train_ops', op)
    for op in [loss, auc_op, aupr_op]:
...
def test(input_size,train_X, train_Y, test_X, test_Y,total_batch,index):
    ...
    with tf.Session() as sess:
        ...
        auc, aupr = sess.run([auc_op, aupr_op],
